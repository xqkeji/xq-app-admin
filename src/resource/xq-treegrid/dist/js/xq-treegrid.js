 ...
 (function () {
  ...
  const bindEdit = (element) => {
    ...
    if (editBtn) {
      editBtn.addEventListener("click", (event) => {
        ...
        const node = xqUtil.parents(targetElement, "tr")[0];
  ...
  const initExpander = (element) => {
    ...
    if (cell) {
      ...
      xqUtil.prepend(cell, expanderTemplate);
      if (!isLeaf(element)) {
    ...
    if (expander) {
      ...
      if (cell) {
        ...
        for (let i = 1; i < depth; i++) {
          xqUtil.prepend(cell, indentTemplate);
        }
      }
    }
  };
  ...
  const bindAdd = () => {
    ...
    if (xqTreegripAdd) {
      xqTreegripAdd.addEventListener("click", () => {
        ...
        for (const check of checks) {
          if (check.checked) {
            const tr = xqUtil.parents(check, "tr")[0];
